...
public:
    ...
    rb_tree_iterator_base(rb_tree_node_base_ptr ptr)
            : node(ptr) {}
...
public:
    ...
    rb_tree_const_iterator(rb_tree_iterator<T> it)
            : rb_tree_iterator_base<T>(it.node) {}
...
public:
    rb_tree()
            : header_(new node_base_type(rb_tree_color::red)), count_(0) {
    ...
    rb_tree(const std::initializer_list<T>& list, const Compare& c = Compare())
            : rb_tree(c) {
