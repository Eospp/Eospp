...
public:
    ...
    constexpr explicit tuple_impl(Args &&... values)
        : tuple_value<I, T>(estd::forward<Args>(values))... {}
    template <typename... Args>
    constexpr explicit tuple_impl(const tuple<Args...> &values)
        : tuple_value<I, T>(estd::get<I>(values))... {}
    template <typename... Args>
    constexpr explicit tuple_impl(tuple<Args...> &&values)
        : tuple_value<I, T>(estd::move(estd::get<I>(values)))... {}
    ...
    tuple_impl &operator=(tuple_impl &&t) {
        swallow{
            (tuple_value<I, T>::operator=(move(static_cast<tuple_value<I, T>>(t).get())), true)...};
        return *this;
    }
};
...
class tuple {
public:
    using value_type = tuple_impl<estd::make_index_sequence<sizeof...(T)>, T...>;
