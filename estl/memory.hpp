...
public:
    ...
    unique_ptr_impl(pointer p, Deleter&& d)
            : data_(p, estd::forward<Deleter>(d)) {}
...
public:
    ...
    unique_ptr(pointer p, estd::conditional_t<is_reference_v<D>, D, const D&> d)
            : impl_(p, d) {}
    ...
    unique_ptr(unique_ptr<U, E>&& u) noexcept
            : impl_(u.release(), estd::move(u.get_deleter())) {}
...
public:
    ...
    count_impl(T* ptr, const Deleter& d)
            : ptr_(ptr), deleter_(d) {}

    count_impl(T* ptr, Deleter&& d)
            : ptr_(ptr), deleter_(estd::forward<Deleter>(d)) {}
...
public:
    ...
    shared_count(count_base* impl)
            : impl_(impl) {
    }

    shared_count(shared_count&& rhs) noexcept
            : impl_(rhs.impl_) {
        rhs.impl_ = nullptr;
    }
...
public:
    ...
    weak_count(const weak_count& rhs)
            : impl_(rhs.impl_) {
        if (impl_) {
            impl_->copy_weak_ref();
        }
    ...
    weak_count(weak_count&& rhs) noexcept
            : impl_(rhs.impl_) {
        rhs.impl_ = nullptr;
    }

    weak_count(const shared_count& rhs) noexcept
            : impl_(rhs.impl_) {
        if (impl_) {
            impl_->copy_weak_ref();
        }
...
public:
    ...
    shared_ptr()
            : ptr_(nullptr), refcount_() {}

    shared_ptr(const shared_ptr& rhs)
            : ptr_(rhs.ptr_), refcount_(rhs.refcount_) {}
...
private:
    shared_ptr(const weak_ptr<T>& rhs)
            : ptr_(rhs.ptr_), refcount_(rhs.refcount_.get()) {
    }
