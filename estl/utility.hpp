...
template <typename T>
constexpr T&& forward(estd::remove_reference_t<T>& t) noexcept {
    return static_cast<T&&>(t);
}

template <typename T>
constexpr T&& forward(estd::remove_reference_t<T>&& t) noexcept {
    static_assert(!estd::is_lvalue_reference<T>::value,
                  "cant' cast rvalue refernce to lvalue reference");
...
struct make_integer_sequence_helper {
    using type = typename make_integer_sequence_helper<T, N - 1, N - 1, I...>::type;
};
template <typename T, size_t... I>
struct make_integer_sequence_helper<T, 0, I...> {
    using type = integer_sequence<T, I...>;
};

template <typename T, T num>
using make_integer_sequence = typename make_integer_sequence_helper<T, num>::type;
